...
public:
	...
	Matrix<Vector3f, 2, 1> get_segmentfm(Matrix<Vector3f, 2, 1> p_ten, Matrix<Vector3f, 2, 1> f_tend, int i, int n, Matrix<float, 2, 1> weights, Matrix<Vector3d, 2, 1> prev_MF, float l, float th) {
		...
		//Vector3f MFi = prev_coord.cross(prev_MF[1][0]);

		Vector3f M1_rel = ai.cross(F1_rel);
		Vector3f M2_rel = bi.cross(F2_rel);

		Vector3f prevF = T_mat * prev_MF[1][0]; //puts previous force in terms of i-1 reference frame
		Vector3f prevM = T_mat * prev_MF[0][0]; //previous moment in terms of i-1 reference frame
